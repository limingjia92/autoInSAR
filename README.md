...
* **Linux/macOS**: Recommended for ISCE2 compatibility.
...
File: `~/.netrc`
    ...
    machine s1qc.asf.alaska.edu login <USERNAME> password <PASSWORD>
...
**Options:** `search`, `download`, `orbit`, `dem`, `xml`, `isce`, `post`, `all` (default).
    ...
    # Example: Only run post-processing (plotting)
...
| `--platform`       | String | No       | Satellite platform (Default: `Sentinel-1`). |
...
*\* Note: You must provide either `--event_date` OR both `--reference_date` and `--secondary_date`.*
...
After a successful run, the following directory structure is created:
    ...
    └── results/            # Final GeoTIFFs and Plots
